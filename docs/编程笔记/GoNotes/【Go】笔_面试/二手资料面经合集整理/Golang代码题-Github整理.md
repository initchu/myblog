...
### 7. 通过指针变量 p 访问其成员变量 name，有哪几种方式？
...
```go
var p *intfunc foo() (*int, error) {	var i int = 5	return &i, nil}func bar() {	//use p	fmt.Println(*p)}func Test43(t *testing.T) {	p, err := foo()	if err != nil {		fmt.Println(err)		return	}	bar()	fmt.Println(*p)}
```
...
```go
//A
type Integer intfunc (a Integer) Add(b Integer) Integer {
    return a + b
}
//B
type Integer intfunc (a Integer) Add(b *Integer) Integer {
...
func Test78(t *testing.T) {
    ...
    wg.Add(2)
    var d data
    go func() {		defer wg.Done()		d.test("read")	}()	go func() {		defer wg.Done()		d.test("write")	}()	wg.Wait()}
```
...
```go
func Test93(t *testing.T) {	var src, dst []int	src = []int{1, 2, 3}	dst = append(dst, src...)	fmt.Println(dst)}
```
...
```go
type User struct {   Name string} func (u *User) SetName(name string) {    u.Name = name    fmt.Println(u.Name)}type Employee Userfunc main() {    employee := new(Employee)    employee.SetName("Jack")}
```
...
> 更新: 2022-03-17 22:06:13  
> 原文: <https://www.yuque.com/xiaoshan_wgo/codingnotes/zkt3vp>